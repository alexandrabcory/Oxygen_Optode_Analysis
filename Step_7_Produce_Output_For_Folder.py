...
def generateAllOutputByFolder(inputFolderPath, outputFolderPath_1, outputFolderPath_2, csv_calCurveParamsByOptode, resolutionDecreaseFactor, bins, optodeNumber, daysToAdd, maxO2ToPlot, site):
    ...
    #         (9) maxO2ToPlot
    #         (10) site
    # Get the crop coordinates for the first image in the folder. 
    first_cr2_file = next((f for f in os.listdir(inputFolderPath) if f.endswith('.CR2')), None)
    ### Getting cropping coordinates- will take the first one in the folder and maintain this for all files in the folder
    
    ### Getting a dataframe of optode-specific measurements
    ...
    count = 1
    ### Counting the number of files total
    for filename in os.listdir(inputFolderPath):
    
        if filename.endswith('.CR2'):
            count = count + 1

    ### Now, iterating through all files in the folder and generating histograms
    for filename in os.listdir(inputFolderPath):
    
    # Checking if the file is a CR2 file
        if filename.endswith('.CR2'):
        
            # generate and save a heat map
            step4.plotRaster(filename, inputFolderPath, outputFolderPath_1, csv_calCurveParamsByOptode, resolutionDecreaseFactor, xMin, xMax, yMin, yMax, optodeNumber, maxO2ToPlot, daysToAdd, )
            
            # generate and save a histogram
            step5.createAndSaveHistogramAndRaster(filename, inputFolderPath, outputFolderPath_1, csv_calCurveParamsByOptode, resolutionDecreaseFactor, bins, optodeNumber, xMin, xMax, yMin, yMax, daysToAdd, maxO2ToPlot)
            # generate and save O2 concentration csv file
            step6.saveO2ConcToCSVs(filename, inputFolderPath, outputFolderPath_2, csv_calCurveParamsByOptode, resolutionDecreaseFactor, optodeNumber, xMin, xMax, yMin, yMax, daysToAdd, site)
            
            # including a print statement to track which images have been processed
            print("finished " + str(fileNumber) + " of " + str(count-1))
